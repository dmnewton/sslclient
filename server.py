...
class PeerCertWSGIRequestHandler( werkzeug.serving.WSGIRequestHandler ):
    ...
    def make_environ(self):
        ...
        x509_binary = self.connection.getpeercert(False)
       # x509 = OpenSSL.crypto.load_certificate( OpenSSL.crypto.FILETYPE_ASN1, x509_binary )
        environ['peercert'] = x509_binary
        return environ
app = Flask(__name__)
...
app_key = './server-key.pem'
app_key_password = None
app_cert = './server-cert.pem'
...
if __name__ == "__main__":
    app.run( host='::',port=8080,ssl_context=ssl_context, request_handler=PeerCertWSGIRequestHandler )